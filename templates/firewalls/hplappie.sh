...
#
#  Generated Sat Dec 21 13:54:28 2024 AEST by acas
#
# files: * hplappie.fw /etc/hplappie.fw
#
...
script_body() {
    ...
    # types; ping request
    $IPTABLES -N Cid9413X1482181.0
    for i_eth0 in $i_eth0_list
    do
    test -n "$i_eth0" && $IPTABLES -A OUTPUT  -d $i_eth0   -m state --state NEW  -j Cid9413X1482181.0 
    done
    $IPTABLES -A Cid9413X1482181.0 -p icmp  -m icmp  --icmp-type 3  -j ACCEPT
    $IPTABLES -A Cid9413X1482181.0 -p icmp  -m icmp  --icmp-type 0/0   -j ACCEPT
    $IPTABLES -A Cid9413X1482181.0 -p icmp  -m icmp  --icmp-type 8/0   -j ACCEPT
    $IPTABLES -A Cid9413X1482181.0 -p icmp  -m icmp  --icmp-type 11/0   -j ACCEPT
    $IPTABLES -A Cid9413X1482181.0 -p icmp  -m icmp  --icmp-type 11/1   -j ACCEPT
    $IPTABLES -A Cid9413X1482181.0 -p tcp -m tcp  --dport 22  -j ACCEPT
...
case "$cmd" in
    start)
        log "Activating firewall script generated Sat Dec 21 13:54:28 2024 by acas"
        check_tools
         prolog_commands 
        check_run_time_address_table_files
