...
verify_interfaces() {
    :
    echo "Verifying interfaces: end0 lo"
    for i in end0 lo ; do
...
script_body() {
    ...
    echo "Rule 3 (global)"
    # 
    $IPTABLES -N Cid8071X2368231.0
    $IPTABLES -A OUTPUT  -d 192.168.2.1   -m state --state NEW  -j Cid8071X2368231.0
    $IPTABLES -A Cid8071X2368231.0 -p icmp  -m icmp  --icmp-type 3  -j ACCEPT
    ...
    for i_end0 in $i_end0_list
    do
    test -n "$i_end0" && $IPTABLES -A INPUT -p tcp -m tcp  -s $i_end0   --dport 1514:1515  -m state --state NEW  -j ACCEPT 
    done
    ...
    for i_end0 in $i_end0_list
    do
    test -n "$i_end0" && $IPTABLES -A OUTPUT  -d $i_end0   -j RULE_6 
    done
